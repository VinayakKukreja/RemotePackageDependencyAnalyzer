...
namespace CodeAnalysis
{
    ...
    public class Repository
    {
        ...
        public ITokenCollection semi { get; set; }
        ...
        public List<Elem> locations { get; set; } = new List<Elem>();
    ...
    public class PushStack : AAction
    {
        ...
        public override void doAction(ITokenCollection semi)
        {
            ...
            if (displaySemi)
            {
                ...
                var indent = new string(' ', 2 * repo_.stack.count);
    ...
    public class PopStack : AAction
    {
        ...
        public override void doAction(ITokenCollection semi)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < repo_.locations.Count; ++i)
                {
                    var temp = repo_.locations[i];
            ...
            if (displaySemi)
            {
                ...
                var indent = new string(' ', 2 * (repo_.stack.count + 1));
                Console.Write("{0}", indent);
    ...
    public class PrintFunction : AAction
    {
        ...
        public override void display(ITokenCollection semi)
        {
    ...
    public class SaveDeclar : AAction
    {
        ...
        public override void doAction(ITokenCollection semi)
        {
            ...
            if (!repo_.typeTable.contains(elem.type))
                return;
            var srcFile = repo_.typeTable.getFile(elem.type, elem.nameSpace);
            if (srcFile == "")
                return;
    ...
    public class DetectFunction : ARule
    {
        public static bool isSpecialToken(string token)
        {
            ...
            foreach (var stoken in SpecialToken)
                if (stoken == token)
                    return true;
            return false;
        }
    ...
    public class BuildTypeAnalyzer
    {
        ...
        public BuildTypeAnalyzer(ITokenCollection semi)
        {
            repo.semi = semi;
        }
        ...
        public virtual Parser build()
        {
            ...
            var push = new PushStack(repo);

            // capture namespace info
            var detectNS = new DetectNamespace();
            detectNS.add(push);
            parser.add(detectNS);

            // capture class info
            var detectCl = new DetectClass();
            detectCl.add(push);
            parser.add(detectCl);
            ...
            var anon = new DetectAnonymousScope();
            anon.add(push);
            parser.add(anon);
            ...
            var leave = new DetectLeavingScope();
            var pop = new PopStack(repo);
            leave.add(pop);
            parser.add(leave);
    ...
    {
        private readonly Repository repo = Repository.getInstance();

        public BuildDepAnalyzer(ITokenCollection semi)
        {
            repo.semi = semi;
        }
        ...
        public virtual Parser build()
        {
            ...
            // save declarations
            var Decl = new DetectDeclar();
            var saveDecl = new SaveDeclar(repo);
            Decl.add(saveDecl);
            parser.add(Decl);

            var Decl2 = new DetectParamDeclar();
            Decl2.add(saveDecl);
            parser.add(Decl2);

            var Decl3 = new DetectBaseClass();
            Decl3.add(saveDecl);
            parser.add(Decl3);
