...
namespace Lexer
{
    ...
    public class Toker
    {
        ...
        public bool doReturnComments { get; set; }
       ...
       */
        ...
        {
            var src = new TokenSourceFile(context_);
            context_.src = src;
       ...
       */
        private bool overwrite(string tok)
        {
            if (isWhiteSpace(tok))
                return true;
        ...
        public static bool isWhiteSpace(string tok)
        {
        ...
        public static bool isNewLine(string tok)
        {
            if (tok == null || tok.Length == 0)
                return false;
            return tok[0] == '\n';
        ...
        public static bool isAlphaNum(string tok)
        {
            if (tok == null || tok.Length == 0)
                return false;
        ...
        public static bool isPunctuator(string tok)
        {
            if (tok == null || tok.Length == 0)
                return false;
            //return (!isWhiteSpace(tok) && !isAlphaNum(tok));
            var test = isWhiteSpace(tok) || isNewLine(tok) || isAlphaNum(tok);
        ...
        public static bool isSingleLineComment(string tok)
        {
        ...
        public static bool isMultipleLineComment(string tok)
        {
            if (tok == null || tok.Length < 2)
                return false;
            if (tok[0] == '/' && tok[1] == '*')
        ...
        public static bool isDoubleQuote(string tok)
        {
            if (tok == null || tok.Length == 0)
                return false;
            var ch = tok[0];
        ...
        public static bool isSingleQuote(string tok)
        {
            if (tok == null || tok.Length == 0)
                return false;
            return tok[0] == '\'';
    ...
    public abstract class TokenState : ITokenState
    {
        ...
        {
            var first = context_.src.peek();
            var second = context_.src.peek(1);
            var chFirst = (char) first;
            var chSecond = (char) second;
        ...
        public bool isDoubleQuote()
        {
            ...
            if (ch == '@')
            {
                var nxt = (char) context_.src.peek(1);
                return nxt == '\"';
            }
        ...
        {
            var test = isWhiteSpace() || isNewLine() || isAlphaNum();
       ...
       * \\"  is not escaped
       */
        protected bool isEscaped(string tok)
        {
            var size = tok.Length;
            if (size < 2)
                return false;
    ...
    public class DoubleQuoteState : TokenState
    {
        ...
        public override string getTok()
        {
            ...
            var nxt = (char) context_.src.peek();
            if (nxt == '\"' && tok[0] == '@')
                tok.Append((char) context_.src.next());
    ...
    {
        private readonly List<int> charQ_ = new List<int>(); // enqueing ints but using as chars
        private readonly TokenContext context_;
    ...
    internal class DemoToker
    {
       ...
       */
        private static bool testTokerWithComments(string path)
        {
            var toker = new Toker();
            toker.doReturnComments = true;

            var fqf = Path.GetFullPath(path);
            if (!toker.open(fqf))
            {
                Console.Write("\n can't open {0}\n", fqf);
                return false;
        ...
        private static bool testTokerWithoutComments(string path)
        {
            var toker = new Toker();
