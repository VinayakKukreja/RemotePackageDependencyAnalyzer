...
namespace MessagePassingComm
{
    ...
    public class CommMessage
    {
        ...
        {
            [EnumMember] connect, // initial message sent on successfully connecting
            [EnumMember] request, // request for action from receiver
            [EnumMember] reply, // response to a request
            [EnumMember] closeSender, // close down client
            [EnumMember] closeReceiver // close down server for graceful termination
        }
        ...
        [DataMember] public MessageType type { get; set; } = MessageType.connect;
        ...
        [DataMember] public List<string> arguments { get; set; } = new List<string>();

        [DataMember] public int threadId { get; set; } = Thread.CurrentThread.ManagedThreadId;

        [DataMember] public string errorMsg { get; set; } = "no error";
        ...
        {
            var msg = new CommMessage(MessageType.request);
            msg.type = type;
            msg.to = to;
            msg.from = from;
            msg.author = author;
